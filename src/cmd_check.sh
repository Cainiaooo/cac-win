...
" "$_cj" 2>/dev/null || true)
    ...
    local provider_policy signal_policy provider_parent_keys provider_child_keys provider_settings_scan provider_settings_keys
    ...
    provider_settings_scan=$(_provider_routing_settings_scan "$env_dir" 2>/dev/null || true)
    ...
    if [[ "$verbose" == "true" ]]; then
        ...
        if [[ -n "$provider_settings_scan" ]]; then
            while IFS=$'\t' read -r _sg_file _sg_key _sg_category; do
                ...
                echo "    $(_yellow "⚠") ${_sg_file/#$HOME/~}  $_sg_key (${_sg_category:-unknown})"
