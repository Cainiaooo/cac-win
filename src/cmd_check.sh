...
const expected=new Intl.DateTimeFormat(ro.locale||undefined,{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false,timeZone:tz}).format(d);
const localeOk=!locale || ro.locale===locale || (ro.locale||'').startsWith(locale + '-u-');
const tzOk=!tz || ro.timeZone===tz;
const formatOk=!tz || actual===expected;
process.stdout.write(tzOk && localeOk && formatOk ? 'ok' : JSON.stringify({timeZone:ro.timeZone||'', locale:ro.locale||'', actual, expected}));
" 2>/dev/null || true)
        (( _id_total++ )) || true
