...
| `--no-link` | Used with `--clone`. Copy files and perform a one-time settings merge for independent customization. |
...
| `--provider-routing <managed\|warn\|preserve>` | Controls whether known provider-routing env vars are hidden before launch, preserved with warnings, or explicitly preserved. Defaults to `managed` for proxy envs and `warn` otherwise. |
| `--signal-guard <warn\|strict>` | Warn or block before launch when risky local provider-routing/timezone/settings signals are visible. Defaults to `warn`. |
| `--sanitize-provider-routing` | With `--clone`, remove known provider-routing keys from cloned settings. This is the default. |
| `--preserve-provider-routing` | With `--clone`, keep known provider-routing keys and set provider routing to `preserve` if no explicit policy was supplied. |
...
Use these when you need explicit launch-time handling for Claude Code provider routing signals.

```bash
cac env set work provider-routing managed   # hide known provider-routing env vars
cac env set work provider-routing warn      # keep them, but report visible key names
cac env set work provider-routing preserve  # explicit opt-out from local management
cac env set work signal-guard warn
cac env set work signal-guard strict
```

`strict` blocks startup when provider-routing signals are visible in risky local combinations or inside Claude settings. Diagnostics only print key names, not secret values.
### Timezone and locale
...
Detach an older copied clone from its source settings.

```bash
cac env detach work
```

This removes `clone_source` and `settings.override.json`, preserving the current merged `settings.json` as the environment's independent settings. New `--clone --no-link` environments are already detached at creation time.
...
- Signal guard (provider-routing policy, settings scan, Node/Bun runtime probes)
