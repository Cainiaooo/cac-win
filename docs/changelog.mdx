...
- **Persona presets** [P1]: `--persona macos-vscode|macos-cursor|macos-iterm|linux-desktop` injects terminal environment variables (`TERM_PROGRAM`, `__CFBundleIdentifier`, `CURSOR_TRACE_ID`, etc.) and hides Docker signals (`/.dockerenv`, `/proc/1/cgroup`). Docker/server environments now look like desktop developer setups.
