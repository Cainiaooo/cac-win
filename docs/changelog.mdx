...
**Fix: auto-repair plugin marketplace `installLocation` after env clone**

`~/.claude/plugins/known_marketplaces.json` records each marketplace's install directory as an **absolute path**. When an environment is created with `--clone` (or migrated from an older layout), that file is copied verbatim, so the recorded `installLocation` still points at the source config dir (e.g. the host `~/.claude`). Claude Code validates that the path lies inside the active `CLAUDE_CONFIG_DIR`'s `plugins/marketplaces`, and otherwise rejects the marketplace with `corrupted installLocation` — surfaced when running `/plugin`.

- `_ensure_initialized` (runs on every `cac` command) now rewrites each env's stale `installLocation` to that env's own `plugins/marketplaces` directory. The fix self-heals on the next `cac` invocation — no `npm install` or environment re-creation required.
- The rewrite only fires when the marketplace files actually live inside the env dir, so it never creates a dangling pointer.
- Link-mode clones (on macOS/Linux, `--clone` symlinks `.claude/plugins` to the shared source) are skipped, so the host or other linked envs are never rewritten through the symlink. Windows is always copy mode and unaffected.

Bumps `CAC_VERSION` to `1.5.8-win.7`.

## v1.5.8-win.5
<sub>2026-04-28</sub>

**Fix: disable `cac self update` in this fork**

`cac self update` was inherited from upstream and still pointed to `https://raw.githubusercontent.com/nmhjklnm/cac/master/cac`. Running it on a cac-win install would silently overwrite the fork's `cac` binary with upstream's version, discarding every Windows-specific patch (T01–T07 from the 2026-04-28 sync round, plus all earlier fork divergence).

- Replace the update body with a clear error pointing to the correct flow: `git pull` + `scripts/install-local-win.ps1`.
- Remove the dead `_SELF_REPO` constant and upstream raw-URL references from `cmd_self.sh`.
- Update `cac self help` to surface the fork-specific update guidance instead of an `update` subcommand that silently destroyed the install.

## v1.5.8-win.4
<sub>2026-04-28</sub>

**Chore: align fork version metadata and add CI consistency check**

This release is the wrap-up of the 2026-04-28 upstream sync round (T01–T05 from `docs/upstream-merge-plan-2026-04-28.md`; T06 fish shell PATH integration was deliberately skipped).

- `package.json` version is now `1.5.8-win.4`, matching `CAC_VERSION` in `src/utils.sh` and the version embedded in the built `cac` binary. Previously the fork shipped `package.json=1.0.0` while runtime reported `1.5.8`, which broke any tooling that relied on either source.
- New CI job `version-check` fails the build whenever `package.json`, `src/utils.sh:CAC_VERSION`, and the `CAC_VERSION="..."` line inside `cac` drift apart.
- Removed `.github/workflows/npm-publish.yml`. This fork is not published to npm; users install via local clone (see README). The publish workflow only existed as upstream inheritance and could fire by accident on a `v*` tag.
- The README now states the version-source-of-truth and the explicit no-npm-publish stance.
...
**Fix: env check no longer reports unverified exit IP as healthy**

When a proxy is configured and reachable but all exit-IP probes fail (DNS pollution, blocked endpoints, captive portals), `cac env check` previously displayed `✓ exit IP run again to detect exit IP` and reported "all good" overall. This was misleading — the diagnostic genuinely could not verify proxy traffic, and that should surface as a problem.

- Change the failed-probe branch to render in red (`✗ exit IP unable to verify via proxy`).
- Record the failure in the `problems` list so the final summary reflects it.

This is a diagnostic-accuracy fix, not a proxy connectivity change. Healthy proxies that pass exit-IP detection are unaffected.

Ports the second hunk of upstream `902728b` (deferred from T02).

## v1.5.8-win.2
<sub>2026-04-28 (fork)</sub>

**Fix: SOCKS5 diagnostics were using local DNS instead of proxy DNS**

`cac env create` timezone detection and `cac env check` exit-IP / TZ verification previously called `curl --proxy socks5://...` directly. With SOCKS5, that makes `curl` resolve probe domains **locally** instead of through the remote proxy.

In environments with DNS pollution, split-DNS, captive enterprise resolvers, or synthetic test ranges such as `198.18.0.0/15` (Clash/mihomo), that created a bad failure mode:
...
- Added an internal `_curl_proxy_url` helper that rewrites curl-based SOCKS5 probes to `socks5h://...`, forcing remote DNS resolution. HTTP/HTTPS proxies pass through unchanged.
- Wrapped all three `curl --proxy` probes in `cmd_check.sh` (one-shot metadata, exit-IP retry loop, TZ fallback) and the timezone geo-detect in `cmd_env.sh`. Localhost relay probes are intentionally untouched (not SOCKS5).
- Added `scripts/test-socks5h-probes.sh` regression test and wired it into the CI build-check job.

Ports upstream `00d2796` (fork sync).

## v1.5.8-win.1
<sub>2026-04-28 (fork)</sub>

**Fix: legacy SOCKS5 proxy URI normalization across startup paths**

Proxies stored as `socks5://host:port:user:pass` (the legacy 4-segment form embedded after the protocol prefix) are now normalized to `socks5://user:pass@host:port` before connectivity checks, relay startup, and environment listings. The generated wrapper inlines the normalization logic since it cannot call helpers from the main `cac` script at startup time. Ports upstream `902728b` (proxy hunks only — the exit-IP semantic change is deferred) and `8c34ebf`.

## v1.5.8-win.0
<sub>2026-04-28 (fork)</sub>

**Fix: `--clone` now inherits `~/.claude/agents/`**

`cac env create --clone` previously copied `commands`, `hooks`, `skills`, and `plugins`, but missed `agents/`. Newly cloned environments lost subagent configuration. `agents` is now part of `clone_dirs` so subagents follow the rest of the cloned config. Top-level `cac env help` also documents `--clone [source]` and `--no-link`. Ports upstream `966303d` (v1.5.7) and `b5883c2`.
