...
| `settings.json` | Layered merge via `settings.override.json` on each startup | One-time merged copy at creation |
...
Settings behave differently depending on clone mode.

In symlink mode, settings use a layered merge system:
...
The merge happens on each `claude` startup, so if you update your base settings, linked cloned environments pick up the changes automatically.
...
With `--no-link`, cac performs that merge once during creation and then stops tracking the source. The copied environment has its own independent `settings.json`, so later changes to `~/.claude/settings.json` do not affect it.

If you created a copied environment with an older cac version and it still picks up source settings changes, detach it:
