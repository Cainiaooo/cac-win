...
| `settings.json` | 每次启动通过 `settings.override.json` 分层合并 | 创建时一次性合并后独立 |
...
合并在每次 `claude` 启动时自动执行，所以更新基础 settings 后，符号链接克隆环境会自动同步。
...
使用 `--no-link` 时，cac 只会在创建时执行一次合并，之后不再跟踪源配置。复制出来的环境拥有独立的 `settings.json`，后续修改 `~/.claude/settings.json` 不会影响它。

如果你用旧版本 cac 创建过复制环境，且它仍会同步源 settings，可以手动解除关联：
